...
std::vector<double> Controllers::PIDController(const double PI, const double SAMPLE_TIME, double* qm,
				...
				std::free(dqm); std::free(dotePos); std::free(ePos); std::free(sq); std::free(dotqr); std::free(s); 
				std::free(qd); std::free(dqd); std::free(dotsigma); std::free(ePosi); std::free(vel);
